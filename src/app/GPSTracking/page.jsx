...
const convertToDMS = (coordinate, isLatitude) => {
  ...
  // Use correct degree symbol
  return `${degrees}°${minutes}'${seconds}"${direction}`
}

// Enhanced Google Maps Component with custom markers
const GoogleMapsComponent = ({ cabs, selectedCab, onCabSelect, addresses }) => {
  ...
  }, [cabs, selectedCab, isGoogleMapsLoaded, addresses])
  ...
  const createMarkers = () => {
    ...
    cabs.forEach((cab) => {
      ...
      const addr = addresses?.[cab.imei]?.address
      const addressHtml = addr
        ? `<div style="margin-top:8px; font-size:12px; color:#374151; line-height:1.3;">
             <strong style="color:#111827;">Address:</strong> ${addr}
           </div>`
        : `<div style="margin-top:8px; font-size:12px; color:#6B7280;">Resolving address…</div>`
...
const DynamicGPSTracking = () => {
  ...
  // Reverse geocoding state (addresses per IMEI)
  const [addresses, setAddresses] = useState({}) // { [imei]: { address, timestamp } }
  const lastGeocodeTimesRef = useRef({}) // throttle per IMEI
  ...
  const GOOGLE_MAPS_API_KEY =
    ...
    // fallback to the same key used by Maps loader above
    "AIzaSyAKjmBSUJ3XR8uD10vG2ptzqLJAZnOlzqI"
  ...
  const initializeWebSocket = () => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        console.log(`ðŸ“¡ Subscribing to ${validCabs.length} cab IMEIs:`)

        validCabs.forEach((cab) => {
          const subscribeMessage = JSON.stringify({ imei: cab.imei })
          ws.send(subscribeMessage)
          console.log(`   âœ“ Subscribed to IMEI: ${cab.imei} (${cab.cabNumber})`)
        })

        if (validCabs.length === 0) {
          console.log("âš ï¸  No valid IMEIs found to subscribe to")
        }
      }
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          console.log("ðŸ›°ï¸  Parsed GPS Data:", gpsData)

          if (!gpsData.imei || gpsData.lat === undefined || gpsData.lon === undefined) {
            console.warn("âš ï¸  Invalid GPS data structure:", gpsData)
            return
          }
          ...
          setCabs((prevCabs) => {
            const updatedCabs = prevCabs.map((cab) => {
              if (cab.imei === gpsData.imei) {
                console.log(`ðŸŽ¯ Updating cab ${cab.cabNumber} (${gpsData.imei}):`)
                console.log(`   ðŸ“ Old position: ${cab.location?.lat}, ${cab.location?.lng}`)
                console.log(`   ðŸ“ New position: ${gpsData.lat}, ${gpsData.lon}`)
                ...
                console.log(`   âœ… Updated cab data:`, {
          ...
          // Trigger reverse geocoding for this IMEI with cooldown (60s)
          try {
            const imei = gpsData.imei
            const lat = gpsData.lat
            const lng = gpsData.lon
            if (typeof lat === "number" && typeof lng === "number") {
              maybeGeocode(imei, { lat, lng })
            }
          } catch {}
        } catch (parseError) {
          console.error("âŒ Error parsing GPS data:", parseError)
      ...
      ws.onclose = (event) => {
        ...
        if (reconnectAttempts < 5) {
          ...
          console.log("âŒ Max reconnection attempts reached")
      ...
      console.error("âŒ Failed to initialize WebSocket:", err)
      setWsConnected(false)
  ...
  useEffect(() => {
    if (cabs.length > 0 && !wsRef.current) {
      console.log(`ðŸš€ Initializing WebSocket with ${cabs.length} cabs`)
      initializeWebSocket()
    }
  ...
  const handleCabSelect = (cab) => {
    setSelectedCab(cab)
    // Ensure address resolution when selecting a cab
    if (cab?.imei && cab?.location) {
      maybeGeocode(cab.imei, cab.location)
    }
  }
  ...
  // Helper: reverse geocode with cooldown and caching
  const maybeGeocode = (imei, location) => {
    if (!imei || !location) return
    const now = Date.now()
    const last = lastGeocodeTimesRef.current[imei] || 0
    if (now - last < 60000) return // 60s cooldown per IMEI
    lastGeocodeTimesRef.current[imei] = now

    const { lat, lng } = location
    if (typeof lat !== "number" || typeof lng !== "number") return

    // If we already have a relatively fresh address (5 minutes), skip
    const cached = addresses[imei]
    if (cached && now - (cached.timestamp || 0) < 5 * 60 * 1000) return

    geocodeLatLng(imei, lat, lng)
  }

  const geocodeLatLng = async (imei, lat, lng) => {
    try {
      ...
      if (data.status === "OK" && data.results && data.results.length > 0) {
        ...
        setAddresses((prev) => ({
          ...prev,
          [imei]: { address, timestamp: Date.now() },
        }))
      }
    } catch (e) {
      // silent fail
  ...
  // Auto-trigger geocoding whenever cabs list updates (throttled per IMEI)
  useEffect(() => {
    if (!cabs || cabs.length === 0) return
    cabs.forEach((cab) => {
      if (cab?.imei && cab?.location) {
        maybeGeocode(cab.imei, cab.location)
      }
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 flex flex-col md:flex-row">
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center space-x-2 text-xs text-gray-500 mt-1">
                                  ...
                                  {cab.lastUpdated && <span>â€¢ {new Date(cab.lastUpdated).toLocaleTimeString()}</span>}
                                </div>
                                {/* Address preview */}
                                {cab.location && (
                                  <p className="text-[11px] text-gray-500 mt-1 line-clamp-1">
                                    {addresses[cab.imei]?.address || "Resolving address..."}
                                  </p>
                                )}
                              </div>
                            </div>
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs md:text-sm text-gray-600">ðŸ“ž {driver.phone}</p>
                              <p className="text-xs text-gray-500">ðŸ†” {driver.licenseNo}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-lg border border-gray-100">
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      {selectedCab.location && (
                        <p className="text-gray-700 text-sm">
                          <span className="font-semibold">Address:</span> {addresses[selectedCab.imei]?.address || "Resolving address..."}
                        </p>
                      )}
            ...
            <div className="flex-1 p-4 md:p-6 h-[50vh] md:h-auto md:min-h-[calc(100vh-4rem)]">
              <GoogleMapsComponent cabs={cabs} selectedCab={selectedCab} onCabSelect={handleCabSelect} addresses={addresses} />
            </div>
          </div>
        </div>
